...
public class ProductNotificationService {
    ...
    public Mono<Boolean> processRestockNotification(Long productId) {
        return Mono.fromCallable(() -> {
                    ...
                    imcrementRestockRound(product);

                    ProductNotificationHistory notificationHistory = saveNotificationHistory(product);
                    sendNotificationToUsers(productId, notifications, product, notificationHistory );
                    return true;
                })
    ...
    private void sendNotificationToUsers(Long productId, List<ProductUserNotification> notifications, Product product, ProductNotificationHistory notificationHistory) {
        ...
        for (ProductUserNotification notification : notifications) {
            Integer stock = (Integer) redisTemplate.opsForValue().get("productStock:" + productId);
    ...
    private void imcrementRestockRound(Product product) {
        // 상품을 찾고 재입고 회차를 증가시킴
    ...
    private void isOutOfStock(Integer stock) {
        // 재고가 없는 경우 알림 전송 중단
        if (stock <= 0) {
            throw new RuntimeException("재고가 없습니다. 알림을 전송할 수 없습니다.");
            }
    }
