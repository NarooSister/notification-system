...
public class ProductNotificationService {
    ...
    public Mono<Boolean> processRestockNotification(Long productId) {
        return Mono.fromCallable(() -> {
                    ...
                    if (product == null) {
                        System.out.println("Product 캐시 미스:" + productId);
                        product = productRepository.findById(productId)
                                .orElseThrow(() -> new RuntimeException("Product not found"));
                        redisTemplate.opsForValue().set("product:" + productId, product);
                    } else {
                        System.out.println("Product 캐시 히트:" + productId);
                    }
                    ...
                    if (stock == null) {
                        ...
                        redisTemplate.opsForValue().set("productStock:" + productId, stock);
                    }
                    // 재고가 없는 경우 알림 전송 중단
                    if (stock <= 0) {
                    ...
                    // 상품을 찾고 재입고 회차를 증가시킴 (JPA 블로킹 작업)
                    ...
                    ProductNotificationHistory notificationHistory = new ProductNotificationHistory(productId, product.getTotalRestockRound(), ProductNotificationHistory.Status.IN_PROGRESS);
